...
class ModelSetup:
    ...
    def create_micro_random(self, model_type: Literal["mtp", "reference"]) -> None:
        """랜덤 초기화된 Micro 모델 생성 (테스트용)

        원본 모델 없이 랜덤 가중치로 micro 모델 생성.
        로컬 테스트 및 파이프라인 검증용.
        """
        from weighted_mtp.models.meta_mtp import ModelArgs, Transformer

        print("=" * 70)
        print(f"Creating Random-Initialized Micro {model_type.upper()} Model")
        print("=" * 70)
        print()
        ...
        config_path = target_dir / "configs/config.json"
        ...
        safetensors_path = target_dir / "safetensors/model.safetensors"
        save_file(state_dict, str(safetensors_path))

        total_params = sum(p.numel() for p in model.parameters())
        size_mb = safetensors_path.stat().st_size / (1024**2)
        print(f"✓ Saved: {safetensors_path} ({size_mb:.1f} MB, {total_params:,} params)")

        # SHA256 생성
        sha256_hash = calculate_sha256(safetensors_path)
        sha256_file = target_dir / "safetensors/SHA256SUMS"
        with open(sha256_file, "w") as f:
            f.write(f"{sha256_hash}  model.safetensors\n")
        ...
        metadata = {
            ...
            "type": "test",
            "init": "random",
            "description": f"Random-initialized micro model for testing",
            "parameters": {
                "total": total_params,
                "hidden_size": hidden_size,
                "num_layers": n_layers,
        ...
        print()
        print("=" * 70)
        print(f"✅ Random micro-{model_type} created!")
        print("=" * 70)
        print()
...
Examples:
  ...
  python scripts/setup_models.py --model meta-llama-mtp --steps verify
    ...
    parser.add_argument(
        ...
        default="slice",
        help="Micro model initialization: 'slice' (from source model) or 'random' (new weights)",
    )
    ...
    try:
        ...
        if args.create_micro:
            if args.micro_init == "random":
                setup.create_micro_random(args.micro_type)
            else:
